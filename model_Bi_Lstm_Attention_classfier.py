...
class TextBiLstmAttentionConfig(object):
    """CNN配置参数"""
    ...
    attention_size = 2 * hidden_dim # "注意力层，此参数和lstm隐藏层大小相同，双向则为其2倍"


class TextBi_Lstm_Attention_classfier(object):
    """文本分类，RNN模型"""
    ...
    def rnn(self):
        ...
        with tf.name_scope("rnn"):
            ...
            # 静态LSTM网络的构建
            lstm_fw_cell = tf.contrib.rnn.LSTMCell(self.config.hidden_dim)
            lstm_bw_cell = tf.contrib.rnn.LSTMCell(self.config.hidden_dim)
            # _outputs, _ = tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell, lstm_bw_cell, embedding_inputs, dtype=tf.float32)
        ...
        with tf.name_scope("score"):
            ...
            # fc = tf.nn.relu(fc)
            fc_w = tf.Variable(tf.truncated_normal([2 * self.config.hidden_dim, self.config.num_classes], stddev=0.1), name='fc_w')
